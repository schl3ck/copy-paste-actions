 ...
 * @property {boolean} excludeAllCPAComments wheter or not to include cpa
 * comments in other snippets/selections
 *
 * `true`: do not include
 *
 * default: `true`
 ...
 * selection is edited (by the functions "cut", "save remove", "paste replace"
 * and "insert replace")
 *
 * `2`: all comments are removed, except for the ones that threw a warning
 * @property {string} commentMarker with what should the comment start so that
...
function analyse(dict) {
  ...
  Things in square brackets "[ ]" like "[replace]" are optionally, but don't
  need the square brackets when specified.
  If you include the brackets, it won't work.

  Things separated by commas "," like "this, that" denote options. In this
  example it would be either "this" or "that"

  Line "function" is mandatory

  Line "name" is optional. It is only used if you copy multiple selections at
  once, oherwise you can only copy one
  selection at once and it helps when saving and restoring snippets.

  Line "new" is optional and only used for the functions "cut", "copy" and
  "save".

  function: speciefies what to do
    cut [n], copy [n]: marks the start of a new selection. Copies the selection
                       into the internal clipboard, cut also
                       removes it from their current position
    end [paste, insert]: marks the end of a selection. specify "paste" or
                         "insert" if it ends a paste or insert selection
    ...
    if "replace" or "remove" is specified, the insertion replaces the following
      actions until the function "end" is encountered
    if "n" is specified, only the next "n" actions are affected. The selection
      will end if either "n" actions were included or the "end" function was
      found. If the function "pause" is found, after "n" was specified for a
      start function (cut, copy, save), then the remaining number of actions is
      also paused until either

  name: specifies the name of the snippet. You will be asked, when saving a
  ...
  Blocks (If, Repeat (with Each) and Choose from Menu) are included in the
    selection when at least one action of the block is inside the selection and
    will only be removed, if all actions of the block are inside the selection
  ...
  */
  ...
  function removeFromArray(array, obj, fromIndex) {
    ...
    if (array == null) {
      throw new TypeError(
        '"array" is null or undefined. Can\'t operate on null or undefined',
      );
    }
  ...
  if (!Array.isArray(dict.shortcuts)) {
    dict.shortcuts = [dict.shortcuts];
  }
  if (
    // prettier-ignore
  ...
  if (dict.excludeAllCPAComments !== false) {
    dict.excludeAllCPAComments = true;
  }
  ...
  dict.shortcuts.forEach((shortcut, shortcutIndex) => {
     ...
     * @type {Action[]}
     */
    const actions = shortcut.WFWorkflowActions.map(
      /**
     ...
     */
    let comments;
    // reindex actions to ignore CPA comments
    // (if dict.excludeAllCPAComments == true)
    (function() {
      let index = 0;
      comments = actions.filter(
        /**
         * @param {CommentAction | Action} a
         */
        (a) => {
          const isCPAComment =
            a.WFWorkflowActionIdentifier === "is.workflow.actions.comment"
    ...
    const blocks = actions.filter((action) =>
      blockActionIds.includes(action.WFWorkflowActionIdentifier),
    );
    ...
    comments.forEach((action) => {
      // prettier-ignore
      const textForWarning =
        action.WFWorkflowActionParameters.WFCommentActionText.trim();
    ...
    names.forEach((comments, nameIndex) => {
      ...
      // prettier-ignore
      const basePercentageNames =
        basePercentage
        + percentPerShortcut * 0.2
      ...
      const current = {
        /**
         * @type {Snippet} Could also be `true` if there was an error to
         * suppress the errors from upcoming functions
         */
        snippet: null,
        insert: {
          /**
      ...
      comments.forEach((comment) => {
        if (
          current.hasInsert()
          && !current.snippet
          && /^(pause|resume)/.test(comment.function)
        ) {
          warnings.push({
          ...
          if (current.insert.clipboard) {
            removeFromArray(current.inserts, current.insert.clipboard);
          }
          if (current.insert.snippet) {
            removeFromArray(current.inserts, current.insert.snippet);
          }
        ...
        if (regex.isStartFunc.test(comment.function) && current.snippet) {
          if (!ignoreErrorsForSnippet) {
            const payload =
              comment.name === dict.noSnippetName
                ? {}
                : {
                  payload: {
                    name: comment.name,
            ...
            warnings.push({
              ...
              shortcut: shortcutName,
              // prettier-ignore
              type: `duplicate${current.snippet.isClipboard
                ? "Clipboard"
                : "Snippet"}${
        ...
        if (
          ...
          && evaluateLeftovers(current.snippet, comment.action.modifiedIndex)
        ) {
          current.snippet.finished = true;
        }
        // prettier-ignore
        if (evaluateLeftovers(
          current.insert.clipboard,
          comment.action.modifiedIndex,
        )) {
        ...
        if (
          /^(pause|resume|end)/.test(comment.function)
          && !comment.endsInsert
        ) {
          if (!current.snippet) {
            warnings.push({
            ...
            // when there was an error in a previous comment and the current
            // comment would concern the snippet, ignore it
            return;
            ...
            warnings.push({
              ...
              shortcut: shortcutName,
              // prettier-ignore
              type: current.snippet.isClipboard
        ...
        // ignore dict.excludeCPAComments because we've reindexed the actions
        // to ignore current functions and if it were false, then the other
        // comments can be treated like regular actions
        ...
        // don't add 1 to the end because we want to select only 1 action if
        // we specify "1" and the end is inclusive
        ...
        switch (comment.function) {
          ...
          case "insert":
            (function() {
              let i = (current.insert[
                comment.isClipboard ? "clipboard" : "snippet"
              ] = {
                name: comment.name,
                id: ++insertIdMax,
                removes: comment.removes,
          ...
          case "pause":
            (function casePause() {
              ...
              if (current.snippet === true) {
                ...
                throw new Error(
                  "Bug in detection of function === pause and current.snippet === true",
                );
              }

              // current.snippet exists; already handled before the switch
              // current.snippet.ranges has at least one element from the start
              // function
              // prettier-ignore
              const lastRange =
                current.snippet.ranges[current.snippet.ranges.length - 1];

              if (!lastRange.isComplete()) {
                // it doesn't have an end, add it (it will have a beginning in
                // every case)
                lastRange.end = endsRange;
                if (!lastRange.isValid()) {
                  // end < start => doesn't contain any actions => isn't needed
                  // anymore, but leave it in there for
                  // possible leftovers
                ...
                if (leftover) {
                  current.snippet.leftover.resume = leftover;
                }
              } else {
                ...
                // set snippet to true to save that we had a snippet to still
                // catch further errors but suppress them
                ignoreErrorsForSnippet = current.snippet = true;
                return;
              }

              // prettier-ignore
              current.snippet.functionPositions.pauseResume
                .push(comment.action.index);
            })();
            break;
          case "resume":
            (function caseResume() {
              ...
              if (current.snippet === true) {
                // we have a bug above. we should never get here
                throw new Error(
                  "Bug in detection of function === resume and current.snippet === true",
                );
              }
              ...
              if (lastRange.isComplete()) {
                ...
                // save leftover, overwriting possible old value
                if (leftover) {
                  current.snippet.leftover.pause = leftover;
                }
              } else {
                warnings.push({
                  action: comment.action.index,
          ...
          case "end":
            (function() {
              if (comment.endsInsert) {
                ...
                current.insert[what].functionPositions.end =
                ...
                if (current.snippet === true) {
                  // we have a bug above. we should never get here
                  throw new Error(
                    "Bug in detection of function === end and current.snippet === true",
                  );
                }
      ...
      if (current.snippet && !current.snippet.finished) {
        // prettier-ignore
        current.snippet.finished = evaluateLeftovers(
          current.snippet,
          modifiedActions.length - 1,
        );
        if (
          !current.snippet.finished
          // prettier-ignore
          && !current.snippet.ranges[current.snippet.ranges.length - 1]
            .isComplete()
        ) {
          current.snippet.ranges[current.snippet.ranges.length - 1].end =
            modifiedActions.length - 1;
        }
      }
      if (current.insert.clipboard) {
      ...
      if (current.snippet) {
          ...
          if (range.isValid()) {
            for (let j = range.start; j <= range.end; j++) {
              ...
              if (
                // prettier-ignore
                blockActionIds
                  .includes(modifiedActions[j].WFWorkflowActionIdentifier)
              ) {
              ...
              // prettier-ignore
              current.snippet.uuids.vars
                .add(modifiedActions[j].WFWorkflowActionParameters.UUID);
              // prettier-ignore
              current.snippet.uuids.groups.add(
                modifiedActions[j]
                  .WFWorkflowActionParameters
                  .GroupingIdentifier,
              );
        ...
        // add missing block actions and save only the fully contained blocks
        // for removal by removing all others
        ...
        Object.keys(blockUUIDS).forEach((gi) => {
          ...
          for (let i = 0; i < blocks.length; i++) {
            ...
            if (
              block.WFWorkflowActionParameters.GroupingIdentifier === gi
              && !indices.includes(block.index)
            ) {
              current.snippet.actions.push(block);
              // we do not need to add the grouping identifier again, becaus
              // we've already included it
              // prettier-ignore
              current.snippet.uuids.vars
                .add(block.WFWorkflowActionParameters.UUID);
    ...
    inserts.forEach((insert) => {
      ...
      insert.replacesNActions = insert.removes
        ? actionsToRemove.filter(
            // prettier-ignore
            (a) => a.excludedBy.includes(insert.id)
              && !excludeFunctionPositions.includes(a.action),
        ).length
        : 0;
  ...
  return {
    shortcuts: result,
    warnings: warnings,
    nItems: result.reduce(
      // prettier-ignore
      (acc, val) => acc
        + Object.keys(val.snippets).length
        + Object.keys(val.inserts).length,
      0,
    ),
  };
}
...
function evaluateLeftovers(snippetOrInsert, modifiedIndex) {
  ...
  // loop as long as there is a leftover and it doesn't end in the future
  // (= was processed) and something was done
  while (
    ...
    if (lastRange.isComplete()) {
      ...
      if (leftover.resume != null && !inFuture.resume) {
        doneSomething = true;
        // add 1 because we want to start a new range and the range end was
        // inclusive
        const newStart = lastRange.end + leftover.resume + 1;
        if (newStart > modifiedIndex) {
          // we've not resumed => save remaining leftover. No need to add
          // anything because the range start is
          // inclusive
          leftover.resume = newStart - modifiedIndex;
          inFuture.resume = true;
        } else {
          // two possibilities:
          //     we've just resumed, but not added any action => add new range
          //     (newStart == modifiedIndex)
          //     we've already resumed earlier => add new Range at that point
          //     (newStart < modifiedIndex)
      ...
      if (leftover.pause != null && !inFuture.pause) {
        ...
        if (newPause >= modifiedIndex) {
          ...
          //      (meaning the next shortcut action should be included, but not
          //      this current comment,
          //      as they have the same index)
          //      => do not pause yet
          // add 1 because if they equal we still have to include one action
          // (that is also true for more)
      ...
      // still evaluate the end leftover if we already evaluated the pause
      // leftover to count down the end correctly
