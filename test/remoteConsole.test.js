...
describe("remoteConsole", function() {
  describe("customStringify", function() {
    ...
    it("objects, no recursion", function() {
      expect(customStringify({
        foo: "bar",
        a: 42,
        b: false,
        c: null
      }), "simple object").to.equal('{"foo":"bar","a":42,"b":false,"c":null}');
      expect(customStringify({
        foo: {
          bar: 42
        },
      ...
      }), "nested object").to.equal('{"foo":{"bar":42},"bar":{"qux":{"42":42}}}');
    });
    it("arrays", function() {
      expect(customStringify([1, "foo", 3, false, null]), "simple array").to.equal('[1,"foo",3,false,null]');
      expect(customStringify([1, [22, 33], [123, [456]], true]), "nested array")
        .to.equal("[1,[22,33],[123,[456]],true]");
      expect(customStringify({
        foo: [1234, {
          bar: "bar"
        }]
      }), "object in array in object").to.equal('{"foo":[1234,{"bar":"bar"}]}');
    });
    it("same object twice", function() {
      ...
      expect(customStringify({
        ...
        o2: o,
        o3: [o, o]
      }), "no recursion").to.equal(`{"o":${oStr},"o2":${oStr},"o3":[${oStr},${oStr}]}`);
